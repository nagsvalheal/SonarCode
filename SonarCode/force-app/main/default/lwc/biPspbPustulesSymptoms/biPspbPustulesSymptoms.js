...
import PUSTULES_VALUE from '@salesforce/label/c.BI_PSP_Pustules';
import ERROR_MESSAGE from '@salesforce/label/c.BI_PSP_ConsoleError';
import ZERO_VALUE from '@salesforce/label/c.BI_PSP_Null';
import BLACK_VALUE from '@salesforce/label/c.BI_PSP_BodyColor';
...
import BODY_PARTS_REMOVE from '@salesforce/label/c.BI_PSP_BodyPartsRemove';
import BODY_PARTS_SELECT_ALL from '@salesforce/label/c.BI_PSP_BodyPartsSelectAll';
...
export default class biPspbpustulessymptom extends NavigationMixin(LightningElement) {
	...
	@track buttonText = BODY_PARTS_SELECT_ALL;
	@track clickCount = 0;
	...
	@track humanParts = []
	@track pustulesValue = PUSTULES_VALUE
						...
						if (this.carePlanTemplateName === this.pustulesValue) {
							...
							try {
								...
								if (this.totalElements === 30) {
									...
									this.buttonText = BODY_PARTS_REMOVE;
								}
							} catch (err) {
	...
	updateElementCount() {
		...
		elements.forEach((element) => {
			if (element.style.fill === BLACK_VALUE && this.buttonText === BODY_PARTS_SELECT_ALL) {
				this.totalElements = elements.length;
				element.style.fill = '';
			} else if (this.buttonText === BODY_PARTS_REMOVE && element.style.fill === BLACK_VALUE) {
				element.style.fill = BLACK_VALUE;
			} else {
	...
	changeColor(event) {
		...
		if (isChecked) {
			...
			this.isButtonDisabled = true;
			this.buttonText = BODY_PARTS_SELECT_ALL;
			targetElements.forEach((element) => {
				const dataNameValue = element.getAttribute('data-name');
				element.style.fill = '';
	...
	handleAccept() {
		const elements = this.template.querySelectorAll('.body-part');
		elements.forEach((element) => {
			if (element.style.fill === BLACK_VALUE) {
				this.isButtonDisabled = true;
		...
		try {
			if (this.partsOfBody.length > 0 && parseInt(this.sliderValue, 10) > 0) {
				if (this.insertCount === '1') {
					...
					if (this.lastSymptomId && this.carePlanTemplateName === PUSTULES_VALUE) {
						RECORD_UPDATE_ALLERGY_INTOLERANCE({
							itchinessallrecordupdate: itchinessAllRecordUpdate, bodyParts: this.humanParts
						})
							// Null data is checked and AuraHandledException is thrown from the Apex
