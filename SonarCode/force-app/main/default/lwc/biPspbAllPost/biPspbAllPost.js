  ...
  showToastMsg = false;
	...
	formatPostData(data) {
		...
		return data.map((post) => ({
			...
			checkFollowStatusButton: this.userEnrolleeId === post.BI_PSP_CareProgramEnrollee__c ? false : true,
			commentBox: false,
			...
			postYouName: this.userEnrolleeId === post.BI_PSP_CareProgramEnrollee__c ? label.YOU_LABEL
			...
			CHECK_COMMUNITY_USERNAME()
				.then((result) => {
					if (result === true) {
						...
						this.handleError(label.ERROR_MESSAGE);
					}
				})
				.catch((error) => {
					this.handleError(error.message);
		...
		//Call apex to store emoji saved information
			SAVE_EMOJI({
				...
				.catch((error) => {
					...
					this.handleSaveEmojiError(error);
				});
	}
	// Handle emoji save error
	handleSaveEmojiError(error) {
		this.allPost = this.allPost.map(post => ({ ...post, showEmojiPopup: false }));
	...
	handleSaveEmojiSuccess(result, feedId) {
		...
		if (this.saveEmojiResult && this.saveEmojiResult.length > 0) {
			...
			this.allPost = this.allPost.map(post => this.updatePostAfterReaction(post, feedId, result));
		} else {
	...
	updatePostAfterReaction(post, feedId, result) {
		if (post.Id === feedId) {
      const emojiResult = this.getResultEmojiAndImg(result.BI_PSP_ReactionResult__c);
			return {
				...post,
				showEmojiPopup: false,
				toReact: false,
...
getResultEmojiAndImg(reactionResult) {
  ...
  // Update the popup state for posts
	updatePostPopupState(feedItemId) {
		return this.allPost.map(post => ({
			...post,
			secondPopupClass: post.Id === feedItemId ? "second-popup" : "second-popup hidden",
			showEmojiPopup: false,
			commentBox: false,
			displayHide: ""
		}));
  ...
  // Handle the case when there are no reactions
	...
	processThumbsUpResults(result) {
		...
		this.displayTemplateHeart = false;
		this.displayTemplateThinkingFace = false;
		this.thumbsUpResult = this.updateReactionResults(result, this.userEnrolleeId);
	}
    ...
    TOTAL_REACTIONS({ feedItemId: this.feedItemIdEmoji, reactionType: reactionType })
...
handleReactionResult(result, reactionType) {
  if (result && result.length > 0) {
      ...
      this.displayTemplateFoldedHands = reactionType === label.HANDSFOLDED_VALUE;
      this.displayTemplateHeart = reactionType === label.HEART_VALUE;
      this.displayTemplateThinkingFace = reactionType === label.THOUGHTFUL_VALUE;
      ...
      this[propertyName] = this.updateReactionResults(result, this.userEnrolleeId);
  } else {
      this.handleNoReactions();
  }
}
	...
	updateReactionResults(result, userEnrolleeId) {
		return result.map(item => ({
			...
			reactionYouName: userEnrolleeId === item.BI_PSP_CareProgramEnrolleeEmoji__c
				? label.YOU_LABEL
				: item.BI_PSP_CareProgramEnrolleeEmoji__r.BI_PSP_CommunityUsername__c || label.NO_USERNAME_LABEL
		}));
	}
	...
	handleDeleteComment() {
		...
		document.body.style.overflow = "";
			SOFTDELETE_COMMENT_ITEM({
				...
				.then(() => {
					this.updateCommentsAndPosts(this.feedCommentItemIdToDelete,this.postItemId);
	...
	commentBoxOpen(event) {
		...
		if (COMMENT_OPTIONS_MAP[TITLE] && COMMENT_OPTIONS_MAP[TITLE][BODY]) {
			this.commentOption = this.mapLabelsToOptions(COMMENT_OPTIONS_MAP[TITLE][BODY]);
		} else {
  ...
  // Methods to handle the events, calling the combined method
	...
	// handleFollowUnfollowUser(action) {
	// 	this.isLoading = true;
	// 	let actionMethod = action === 'follow' ? FOLLOW_USER : UNFOLLOW_USER;
	// 	let enrolleeIdParam = action === 'follow' ? 'enrolleeIdToFollow' : 'enrolleeIdToUnFollow';
	// 	let followLabel = action === 'follow' ? label.FOLLOWING_LABEL : label.FOLLOW_LABEL;
	// 	let followPopup = action === 'follow' ? 'followPopupAtFeed' : 'followingPopupAtFeed';
	// 	let followPopupConfirmation = action === 'follow' ? 'followPopupConfirmationAtFeed' : 'followingPopupConfirmationAtFeed';
    
	// 	actionMethod({ [enrolleeIdParam]: this.enrolleeId })
	// 		.then(() => {
	// 			this.allPost = this.allPost.map((post) => ({
	// 				...post,
	// 				checkFollowBtn: post.BI_PSP_CareProgramEnrollee__c === this.enrolleeId ? followLabel : post.checkFollowBtn
	// 			}));
	// 			this.isLoading = false;
	...
	followUserYesBtn() {
		...
		FOLLOW_USER({ enrolleeIdToFollow: this.enrolleeId })
			.then(() => {
				this.allPost = this.allPost.map((post) => ({
					...post,
					checkFollowBtn: post.BI_PSP_CareProgramEnrollee__c === this.enrolleeId ? label.FOLLOWING_LABEL : post.checkFollowBtn
				}));
				this.isLoading = false;
				this.followPopupAtFeed = false;
				this.followPopupConfirmationAtFeed = false;
	...
	unfollowUserYesBtn() {
		...
		UNFOLLOW_USER({ enrolleeIdToUnFollow: this.enrolleeId })
			.then(() => {
				this.allPost = this.allPost.map((post) => ({
					...post,
					checkFollowBtn: post.BI_PSP_CareProgramEnrollee__c === this.enrolleeId ? label.FOLLOW_LABEL : post.checkFollowBtn
				}));
				this.isLoading = false;
				this.followingPopupAtFeed = false;
				this.followingPopupConfirmationAtFeed = false;
	...
	// Handle follow/unfollow user for Emoji
	// handleEmojiFollowUnfollow(action) {
	// 	try {
	// 		this.isLoading = true;
	// 		let actionMethod = action === 'follow' ? FOLLOW_USER : UNFOLLOW_USER;
	// 		let enrolleeIdParam = action === 'follow' ? 'enrolleeIdToFollow' : 'enrolleeIdToUnFollow';
	// 		let followLabel = action === 'follow' ? label.FOLLOWING_LABEL : label.FOLLOW_LABEL;
	// 		let toastMsg = action === 'follow' ? 'showFollowToastMsg' : 'showUnfollowToastMsg';

	// 		actionMethod({ [enrolleeIdParam]: this.followEnrolleeId })
	// 			.then(() => {
	// 				this.allPost = this.allPost.map((post) => ({
	// 					...post,
	// 					checkFollowBtn: post.BI_PSP_CareProgramEnrollee__c === this.enrolleeId ||
