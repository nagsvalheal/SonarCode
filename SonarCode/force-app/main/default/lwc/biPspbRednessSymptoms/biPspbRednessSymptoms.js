...
export default class biPspRednessSymptom extends NavigationMixin(LightningElement) {
		...
		if (data && data !== null) {
			try {
				...
				for (let record of data) {
					...
					if (carePlanTemplate === this.rednessValue) {
						this.carePlanTemplateName = this.rednessValue;
					}
					if (this.carePlanTemplateName === this.rednessValue) {
	...
	connectedCallback() {
		...
		try {
			...
			let mybodyparts = globalThis?.sessionStorage.getItem('redness');
			//This code retrieves data labeled as 'myDataintensityredness' from the session storage without altering custom labels.
			let mybodyinternsity = globalThis?.sessionStorage.getItem('myDataintensityredness');
	...
	handleClickForAccept() {
		...
		let itchinessAllRecordInsert = {
			...
			ValoF: parseFloat(this.valueOfTemperature) || 0, // Convert to float and handle null or invalid values
		...
		try {
			if (this.bodyparts.length > 0 && parseInt(this.sliderValue, 10) > 0) {
				...
				if (this.insertCount === '1') {
					...
					if (this.lastSymptomId && this.carePlanTemplateName === label.REDNESS_VALUE) {
						RECORD_UPDATE_ALLERGY_INTOLERANCE({
							itchinessallrecordupdate: itchinessAllRecordUpdate, bodyParts: this.humanParts
						})
							// Null data is checked and AuraHandledException is thrown from the Apex
						...
						RECORD_INSERT_ALLERGY_INTOLERANCE({
							...
							.then(result => {
								if (result && result !== null) {
									...
									globalThis?.sessionStorage.setItem('countred', this.recordInsertCount);
								}
							})
							.catch(error => {
								this.showToast(label.ERROR_MESSAGE, error.message, label.ERROR_VARIANT);
