...
import TASKS_DATE_OF_TREATMENT from '@salesforce/apex/BI_PSPB_WidgetCtrl.getTasksWithDateOfTreatment';
...
export default class BiPspbReminderWidget extends NavigationMixin(LightningElement)
{
	...
	// Differnce between days are calculated.
	...
	getTooltipText(remainCheck) {
		switch (true) {
			case (remainCheck > 1 && remainCheck !== 3):
				return `Prescription Reminder: ${remainCheck} Days Before`;
			case (remainCheck < -1):
				return `Prescription Reminder: ${Math.abs(remainCheck)} Days Before`;
			case (remainCheck === 3):
				return `Treatment Reminder: 3 Days Before`;
			case (remainCheck === 1):
				return `Treatment Reminder: 1 Day Before`;
			default:
	...
	applySpecialStyling() {
		// Check the condition and apply special styling
	...
	calculateAdditionalDates(treatmentDate) {
		...
		// Log the treatment date to verify it's correct
