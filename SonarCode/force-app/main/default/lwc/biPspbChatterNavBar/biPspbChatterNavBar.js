...
export default class BiPspbChatterNavBar extends LightningElement {
  ...
  renderedCallback() {
    ...
    try {
      ...
      this.showToast(label.ERROR_MESSAGE, err.body.message, label.ERROR_VARIANT); // Result Null/other Exception
    }
  }
    ...
    CHECK_COMMUNITY_USERNAME()
      .then((result) => {
        if (result === true) {
          ...
          this.showToast(label.ERROR_MESSAGE, label.ERROR_MESSAGE, label.ERROR_VARIANT);
        }
      })
      .catch((error) => {
