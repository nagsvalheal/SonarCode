//This components using user body parts and intensity itchiness values store this lwc
//To import library
...
export default class BiPspbItchinessSymptom extends NavigationMixin(LightningElement) {
	...
	// page renders call back all values display 
	...
	connectedCallback() {
		try {
			...
			let myBodyParts = globalThis?.sessionStorage.getItem('myData');
			...
			// Store data labeled as 'my Data intensity' in the session storage without altering custom labels.
			let myBodyIntensity = globalThis?.sessionStorage.getItem('myDataintensity');
			...
			}// Store data labeled as 'symptom last id' in the session storage without altering custom labels.
	...
	handleClickForAccept() {
		...
		let itchinessAllRecordInsert = {
			...
			valuesOfMood: this.moodValues || '', // Use default value if valuesOfMood is null
		...
		try {
			if (this.partsOfBody.length > 0 && parseInt(this.sliderValue, 10) > 0) {
				if (this.insertCount === '1') {
					...
					if (this.lastSymptomId && this.carePlanTemplateName === label.ITCHINESS_VALUES) {
						RECORD_UPDATE_ALLERGY_INTOLERANCE({
							itchinessallrecordupdate: itchinessAllRecordUpdate, bodyParts: this.humanParts
						})
							// Null data is checked and AuraHandledException is thrown from the Apex
